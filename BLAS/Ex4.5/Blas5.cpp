...
void conj_grad_banded(double* A,double* x, double *b,int n,double tol, int max_iter,int k){
    double alpha, beta, rcur, rnext;
    double* r = new double[n];
    double* p = new double[n];
    double* Ap = new double[n];

    F77NAME(dsbmv)('U',n,k,1.0,A,k+1,x,1,0.0,Ap,1); 
    ...
    for (int i=0; i<max_iter;i++){
        ...
        F77NAME(dsbmv)('U',n,k,1.0,A,k+1,p,1,0.0,Ap,1);
...
void generate_reduced_helmholtz_system(double* A,double* f,double* u_an,int n,double a,double b,double lambda,double u0,double un1){
    ...
    double x_i=0.0;
     ...
     for (int i=0; i<n-2;++i){
        ...
        // u_an[i]=sin(M_PI*x_i);
        f[i]=-(lambda+(M_PI*M_PI))*cos(M_PI*x_i);//Forcing function
        u_an[i]=cos(M_PI*x_i);
